import random
import time
from typing import List

import numpy as np
import tntorch as tn
import scipy
import torch

np.random.seed(42)


def generate_basket_path(
        s0: np.ndarray,
        vol: np.ndarray,
        R: np.ndarray,
        d: int = 5,
        r: float = 0.05,
        T: float = 1.0,
):
    """
    N: number of underlying assets
    r: risk-free rate
    T: time horizon
    """

    L = np.linalg.cholesky(R)  # Lower triangular matrix

    Z = np.random.randn(d)  # Independent normals
    dW = np.sqrt(T) * (L @ Z)

    drift = (r - 0.5 * vol**2) * T
    diffusion = vol * dW

    logS_T = np.log(s0) + drift + diffusion
    S_T = np.exp(logS_T)

    return S_T


def generate_correlated_R(N):
    A = np.random.randn(N, N)
    vol = A @ A.T
    D = np.diag(1 / np.sqrt(np.diag(vol)))
    return D @ vol @ D


def vhat_min_call(z_grid, K, d):
    s = np.sum(z_grid, axis=0)
    numerator = -K**(1 + 1j * s)
    denominator = ((-1)**d) * (1 + 1j * s) * np.prod(1j * z_grid, axis=0)
    return numerator / denominator


def phi(z_grid, mu, Sigma):
    z_dot_mu = np.dot(mu, z_grid)
    Sigma_z = Sigma @ z_grid
    quad = np.sum(z_grid * Sigma_z, axis=0)
    return np.exp(1j * z_dot_mu - 0.5 * quad)


def fourier_pricing(
        d: int = 5,
        T: float = 1.0,
        r: float = 0.05,
        K: float = 80.0,
        N: int = 32,
        eta: float = 1.0,
):
    """
    d: underlying assets
    T: time horizon
    r: risk-free rate
    K: strike price
    """

    # Fourier Grid
    alpha = 5 / d

    # Model params
    s0 = np.random.uniform(90, 110, size=d)
    vol = np.random.uniform(0.1, 0.3, size=d)
    R = generate_correlated_R(d)  # Correlation Matrix
    mu = np.log(s0) + (r - 0.5 * vol**2) * T
    Sigma = np.diag(vol) @ R @ np.diag(vol) * T

    # TT-cross
    def entry(*indices):
        j = np.array(indices) - N // 2
        z = eta * j + 1j * alpha

        return torch.from_numpy(phi(-z, mu, Sigma) * vhat_min_call(z, K, d))

    price_tensor = tn.cross(
        function=entry,
        domain=[N]*d,
        eps=1e-3,
        rmax=10,
        max_iter=100,
        early_stopping_patience=5,
    )

    price = np.exp(-r * T) * (eta**d) / (2 * np.pi)**d * price_tensor.sum()
    print("Price:", price.real)
    return price.real


fourier_pricing()
